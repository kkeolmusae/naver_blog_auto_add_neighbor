...
with open(os.getcwd() + "/app/login_info.json", "r") as json_file:
...
keyword_list = st_python.get("inclusion_title")
...
def random_paginator_sleep_time():
...
class NaverBlogAutoAddBuddy:
    ...
    def wait_next_window(self):
        print("start wait")
        WebDriverWait(self.driver, self.wait_page_time).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, ".wrap_my_information"))
        )
        print("start done")

    def wait_page_by_xpath(self, xpath: str):
        return WebDriverWait(self.driver, self.wait_page_time).until(
    ...
    def get_blog_post_list(self, next_page=False):
        ...
        if next_page:
            ...
            self.driver.get(
                "https://section.blog.naver.com/ThemePost.naver?directoryNo=0&activeDirectorySeq=0&currentPage="
                + str(self.current_page),
            )
            # self.wait_page_by_xpath("//a[contains(@class, 'button_next')]").click()
        else:
            self.driver.find_element(By.LINK_TEXT, "주제별 보기").click()
        random_paginator_sleep_time()
        ...
        for blog_i in range(blog_len):
            try:
                ...
                blog_titles = self.driver.find_elements(
                    By.XPATH, "//strong[contains(@class, 'title_post')]"
                )
                ...
                empathy_a_herf = self.wait_page_by_xpath(
                    "//a[contains(@id, 'Sympathy')]"
                )
                empathy_a_herf.click()
                random_sleep_time()
                empathy_iframe = self.wait_page_by_xpath(
                    "//iframe[contains(@id, 'sympathyFrm')]"
                )
                empathy_iframe_id = empathy_iframe.get_attribute("id")
                self.driver.switch_to.frame(empathy_iframe_id)

                # 공감한 이웃 모두 받아오기
                buddy_list = self.driver.find_elements(
                    By.XPATH, "//a[contains(@class, 'btn_buddy')]"
                )
                ...
                for buddy_i in range(len(buddy_list)):
                    ...
                    if buddy_i >= 1:
                        ...
                        empathy_iframe_id = empathy_iframe.get_attribute("id")
                        self.driver.switch_to.frame(empathy_iframe_id)
                    buddy = self.driver.find_elements(
                        By.XPATH, "//div[contains(@class, 'wrap_bloger')]"
                    )[buddy_i]
                    buddy_add_buddy_buttom = buddy.find_elements(
                        By.XPATH, "//a[contains(@class, 'btn_buddy')]"
                    )[buddy_i]
                    user_name = buddy.find_elements(
                        By.XPATH, "//span[contains(@class, 'ell2')]/a"
                    )[buddy_i].text
                    ...
                    if "이웃추가" in buddy_add_buddy_buttom.get_attribute("text"):
                        ...
                        # 서이추 버튼 disable
                        each_buddy_add_button = self.wait_page_by_xpath(
                            "//input[contains(@id, 'each_buddy_add')]"
                        )
                        if each_buddy_add_button.get_attribute("disabled"):
                            self.driver.close()

                        else:
                            self.wait_page_by_xpath(
                                "//label[contains(@for, 'each_buddy_add')]"
                            ).click()
                            random_sleep_time()
                            next_button = self.wait_page_by_xpath(
                                "//a[contains(@class, '_buddyAddNext')]"
                            )
                            next_button.click()
                            random_sleep_time()
                            ...
                            if not is_alert:
                                # 서로이웃 신청 메시지 쓰는 칸 입장
                                textarea = self.wait_page_by_xpath(
                                ...
                                # 다음
                                textarea_popup_next_button = self.wait_page_by_xpath(
                                    "//a[contains(@class, '_addBothBuddy')]"
                                )
                                textarea_popup_next_button.click()
                                random_sleep_time()

                                # 최종 닫기 버튼 클릭
                                finsh_close_button = self.wait_page_by_xpath(
                                    "//a[contains(@class, 'button_close')]"
                                )
    ...
    def login_naver(self):
        ...
        # 3. id 복사 붙여넣기
        elem_id = self.wait_page_by_xpath(
            "//input[contains(@id, 'id') and contains(@placeholder, '아이디')]"
        )
        random_sleep_time()
        elem_id.click()
        ...
        # 4. pw 복사 붙여넣기
        elem_pw = self.wait_page_by_xpath(
            "//input[contains(@id, 'pw') and contains(@placeholder, '비밀번호')]"
        )
        random_sleep_time()
        elem_pw.click()
        random_sleep_time()
